...
describe "Exchange::Conversability" do
  ...
  context "with a big decimal" do
    it "should allow to convert to a currency" do
      BigDecimal("3.25").in(:eur).should be_kind_of Exchange::Money
      BigDecimal("3.25").in(:eur).value.round(2).should == 3.25
    end
    it "should allow to convert to a curreny with a negative number" do
      BigDecimal("-3.25").in(:eur).should be_kind_of Exchange::Money
      BigDecimal("-3.25").in(:eur).value.round(2).should == -3.25
    ...
    it "should allow to do full conversions" do
      mock_api("http://api.finance.xaviermedia.com/api/2012/08/27.xml", fixture('api_responses/example_xml_api.xml'), 3)
      BigDecimal("3.25").in(:eur).to(:chf).should be_kind_of Exchange::Money
      BigDecimal("3.25").in(:eur).to(:chf).value.round(2).should == 3.99
      BigDecimal("3.25").in(:eur).to(:chf).currency.should == :chf
    end
    it "should allow to do full conversions with negative numbers" do
      mock_api("http://api.finance.xaviermedia.com/api/2012/08/27.xml", fixture('api_responses/example_xml_api.xml'), 3)
      BigDecimal("-3.25").in(:eur).to(:chf).should be_kind_of Exchange::Money
    ...
    it "should allow to define a historic time in which the currency should be interpreted" do
      BigDecimal("3.25").in(:chf, :at => Time.gm(2010,1,1)).time.yday.should == 1
      BigDecimal("3.25").in(:chf, :at => Time.gm(2010,1,1)).time.year.should == 2010
      BigDecimal("3.25").in(:chf, :at => '2010-01-01').time.year.should == 2010
    end
    it "should raise a no currency error if the currency does not exist" do
      lambda { BigDecimal("3.25").in(:zzz) }.should raise_error(Exchange::NoCurrencyError, "zzz is not a currency nor a country code matchable to a currency")
    end
  end
end
